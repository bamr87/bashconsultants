...
### Added
- **LinkedIn Community Management API application answer** — `docs/linkedin-api-application.md` records the business-and-product description submitted to LinkedIn for the `w_organization_social` + `r_organization_social` scopes the publisher needs: what the practice is, that the pipeline is first-party (one app, our own company page, no third-party users or resale), the exact endpoints and scopes used and what is deliberately not read or stored, and the human-merge approval gate. Cross-linked from the activation steps in `docs/automation.md`, which previously stated the access requirement with nothing behind it.
