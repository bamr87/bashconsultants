...
### Added
- **shiplog — a publishing tool for developers, plus the LinkedIn API application it backs** — `prototype/shiplog/` is a working prototype that reads what a developer already wrote in a repository (git tags, conventional commits, `CHANGELOG.md` sections, markdown docs) and composes LinkedIn posts from it, so shipped work accumulates into a public track record. Standard-library Python, no dependencies: source discovery, developer-declared audience profiles, deterministic template composition with a filler-and-length guard, a pending/approved/published queue, a local review dashboard (`serve`) that shows each draft beside the exact `POST /rest/posts` payload it becomes, a portfolio view (volume, cadence, streak, topics), an idempotency ledger, and an offline `self-test` whose assertions fail if `publish` ever picks up a draft a human did not approve. The live LinkedIn call is deliberately not stubbed — the app has no API access yet, so `publish` renders the request and stops.
- **LinkedIn Community Management API application answers — repositioned around the developer** — `docs/linkedin-api-application.md` now describes shiplog rather than this site's own company-page publisher, which makes **Profile management the primary use case** (a developer publishing their own work to their own profile under `w_member_social`, each authorizing their own account through their own OAuth consent) with **Page management** secondary (a team, product, or open-source project page the developer administers) and **Page analytics** supporting both (own-content statistics, aggregate, feeding the portfolio view). Carries the boundaries the larger ask depends on: every post approved by the person whose name is on it, no scheduler or unattended mode anywhere, audience **declared in config and never derived** from connections or followers, no automated engagement or messaging, no reading other members' profiles or feeds, no scraping, no resale, no model training. States plainly what is built versus planned, since an overstated capability is what loses a grant after it is issued. Cross-linked from the activation steps in `docs/automation.md`, which previously stated the access requirement with nothing behind it.
- **Screen recording for the LinkedIn submission** — `prototype/shiplog/demo/` reproduces the submitted capture from scratch: `setup_demo.sh` builds a stand-in developer project with real git history, tags, a changelog and docs; `seed_state.sh` seeds publish history and drafts awaiting approval; `record.mjs` drives the live dashboard in Chromium via Playwright and records it. The result (`recording/shiplog-review-gate.mp4`, 33s, 1280x800, H.264) shows a draft composed from the repo's own release tag, the payload it would send, the track record, the declared audiences, and a person clicking Approve — the click really does rewrite the draft on disk. It deliberately does **not** show a post reaching LinkedIn, because the app cannot yet make that call and a fabricated success would misrepresent it to the reviewer. `demo/README.md` documents reproduction and carries a timed narration script.
