...
### Added
- **LinkedIn Community Management API application answer** — `docs/linkedin-api-application.md` records the business-and-product description submitted to LinkedIn for the `w_organization_social` + `r_organization_social` scopes the publisher needs. It separates what ships today (first-party publishing to our own page, the exact endpoints and scopes in use, what is deliberately not read or stored, the human-merge approval gate) from the product roadmap it also discloses: a CMS extension and distribution app other organizations can run against their own pages, with AI agents drafting and critiquing commentary, sentiment and engagement analysis on a page's own content, and industry-news sourcing from public feeds outside LinkedIn. States the policy boundary explicitly — multi-organization use gets its own LinkedIn application and per-admin consent, analysis stays aggregated to the page owner, no member profiling, no resale, no model training on LinkedIn data — so the disclosed intent reads as policy awareness rather than scope creep on a first-party grant. Also carries the form's per-use-case answers: **Page management** (posts, comment replies and thread moderation on our own page, engagement monitoring, all behind the merge-is-approval gate), **Page analytics** (aggregate share and page statistics plus comment-sentiment summaries, scoped to pages we administer, no member profiling or re-identification), and **Profile management** in two variants — a narrow request covering only the authorizing member's own profile, and a decline — with a recommendation to skip it unless founder-profile publishing is actually wanted, since requesting an unused capability invites rejection. Cross-linked from the activation steps in `docs/automation.md`, which previously stated the access requirement with nothing behind it.
