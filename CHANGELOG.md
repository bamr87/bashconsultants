...
### Added
- **zer0-CMS convergence — one headless CMS across four repositories** — `docs/zer0-cms-convergence.md` maps what each repo already contributes (it-journey's `scripts/cms/cms.py` and the `.cms/` contract for analysis; zer0-CMS's Front Matter fork for authoring; zer0-image-generator for media; zer0-mistakes for presentation) and identifies the one missing layer: distribution. The loop the product needs — write, publish, engage, cater — runs `write -> publish` and stops, because the content engine can say a page is thin or stale but cannot say which subject an audience cares about. The design closes it by making distribution a third participant on the existing `.cms/` bus rather than a parallel app with its own content model.
- **`prototype/zer0-distribute/` — the distribution lane** (was `prototype/shiplog/`) — reads `.cms/index/content-index.json` so publishable material is the same rows the authoring surface renders, with a git-and-filesystem fallback for repositories that have no engine. Adds `contract.py` (the bus, read and write), `catering.py` (what to write next, in four lanes), `analytics.py` (an enumerable own-content-only read surface plus ingest), and `media.py` (reuses the preview image zer0-image-generator already produced, emits its brief when absent). Writes back `.cms/distribution/performance.json` and `.cms/distribution/worklists/<date>-catering.md`, deliberately in the engine's own worklist format so both land in one list. Validated end to end against it-journey's real index: 380 files, 330 distributable, three posts' statistics ingested, worklist generated. Self-test extended to ~110 assertions covering the contract's withholding branches, the analytics boundary, and the rule that no subject is ranked on a single observation.
- **LinkedIn API answers rewritten for the converged product** — `docs/linkedin-api-application.md` now describes zer0-CMS, a headless CMS for any business or individual, which makes **Page management and Profile management both primary** and **Page analytics the use case that closes the loop** rather than a reporting add-on. Carries the four boundaries the ask depends on: every post approved by whoever's name is on it, each account authorized by its own owner's consent, audience declared in config and never derived from connections or followers, and everything read back being the author's own aggregate numbers. Recording re-shot against the converged dashboard running on the real 380-page index, showing the catering lane and a real Approve click; it still deliberately does not show a post reaching LinkedIn, because the app cannot make that call yet.
...
- **BASH-CMS — the LinkedIn pipeline packaged as a standalone VS Code extension** — new sibling repository [`bamr87/bash-cms`](https://github.com/bamr87/bash-cms): the whole governed pipeline (content discovery via the `.cms/` contract with frontmatter-scan fallback, draft queue, brand guard, review-panel approval gate, publish with the idempotency ledger, catering lanes, bundled MCP server) as a TypeScript-native, marketplace-shaped extension. File- and wire-compatible with this repo's Python lane — golden fixtures generated by `scripts/features/linkedin/` and `prototype/zer0-distribute/` pin the ledger bytes, payload previews, and worklist markdown byte-for-byte, so both lanes share one queue and one ledger without conflict. This workspace's `.vscode/settings.json` now carries the matching `bashCms.*` values.
- **LinkedIn MCP server — the Claude-native front end to the publisher** — `scripts/features/linkedin/mcp_server.py` (stdlib-only, stdio) exposes the company-page publisher as a Model Context Protocol server so Claude Code can inspect the page, preview a post, stage a governed draft, or publish, wired into a project `.mcp.json`. It follows LinkedIn's "Develop with MCP" reference (`user-info` + `create-post`) but adapts it to this repo: local single-operator stdio reusing the `.env` token instead of a remote HTTP+SSE server running the MCP third-party OAuth flow, and it wraps posting in the repo's governance. Six tools — `linkedin_whoami` / `linkedin_list_posts` / `linkedin_get_post` / `linkedin_preview` are read-only, `linkedin_draft` stages a `drafts/linkedin/` file for human merge, and `linkedin_publish` is OFF by default (needs `LINKEDIN_MCP_ALLOW_PUBLISH=1` plus a per-call `confirm: true`, still runs the brand guard and idempotency ledger). Every incidental library print is redirected off the stdout protocol channel so the JSON-RPC stream stays clean.
